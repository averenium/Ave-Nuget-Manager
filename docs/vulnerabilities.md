...
## HTTP-каталог: база йде першою (#27)

За увімкненим прапорцем `experimentalHttpCatalog` спершу читається **база вразливостей по HTTP** (`nugetVulnerabilityDatabase.ts`), і лише якщо її не вдалося прочитати — усе описане вище. Причина в ціні: без `<auditSources>` CLI робить запит на registration **кожного пакета**, а база відповідає двома запитами до одного документа з дисковим кешем за таймстампованою адресою.

Транзитивні при цьому не губляться: зіставлення йде по `installed` **і** `implicit` з того самого `dotnet list --include-transitive`, який панель запускає для списку пакетів.

Дві відмінності, які варто памʼятати:

- `undefined` від бази означає «прочитати не вдалося» і вмикає CLI; порожній перелік означає «прочитано, збігів немає» — і це відповідь, CLI не запускається;
- CLI шанує `NuGetAudit` / `NuGetAuditMode` проєкту, а цей шлях завжди перевіряє всі отримані пакети, тож знахідок може бути **більше**, ніж у попередженнях `dotnet restore`. У логу такий скан видно рядком `vulnerability scan via HTTP database`.
